...
(function () {
  ...
  // Price formatter (handles either minor or major values)
  const fmtPrice = (val, currency = 'TRY', isMinor = true) => {
    let n = Number(val) || 0;
    if (isMinor) n = n / 100;
    return `${n.toLocaleString('tr-TR', { maximumFractionDigits: 2 })} ${esc(currency)}`;
  };
  ...
  function card (x) {
    ...
    const hasMinor = typeof x.price_minor === 'number';
    const priceStr = hasMinor
      ? fmtPrice(x.price_minor, x.currency || 'TRY', true)
      : fmtPrice(x.price,       x.currency || 'TRY', false);
  ...
  async function load () {
    ...
    try {
      ...
      if (!r.ok) {
        box.innerHTML = `<div class="pad error">İlanlar alınamadı (HTTP ${r.status}).</div>`;
        return;
      }
  ...
  function boot () {
    if (!$('#listings')) return; // not this page
    wirePager();
    load();
  }
